...
@subprog.SubprogReg.new_subprog("split_by_cue",
	help = "split single-piece audio files by cue",
	desc = "split single-piece audio files by cue")
class SubprogSplitByCue(subprog.SubprogWithLogBase):
	...
	def create_argparser(self, subparsers, *ka, **kw):
		...
		ap.add_argument("--transcode", type = str, metavar = "format",
			help = "output audio file format; note using some format may cause "
				"problems (default: do not transcode)")
		return ap

	def _transcode(self, input, output_ext, *, force = None, dry_run = None,
			verbose = None) -> str:
	...
	def _split_by_cue(self, input, cue, *, force = None, dry_run = None,
			verbose = None):
		cmd = ["shnsplit"]
		if force:
			cmd.extend(["-O", "always"])
	...
	def subprog_main(self, args):
		...
		self._split_by_cue(split_input, args.cue, force = args.force,
